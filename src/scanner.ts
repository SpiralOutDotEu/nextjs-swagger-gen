...
function parseJSDoc(content: string): {
  ...
  responses?: { [key: string]: { description: string } };
} {
  ...
  const queryParams: { name: string; description: string }[] = [];
  for (const line of lines) {
  ...
  const pathParams: { name: string; description: string }[] = [];
  for (const line of lines) {
  ...
  const responses: { [key: string]: { description: string } } = {};
...
function extractPathParams(routePath: string): { name: string; description?: string }[] {
  const pathParams: { name: string; description?: string }[] = [];
...
export async function scanRoutes(appDir: string): Promise<RouteInfo[]> {
  ...
  return Promise.all(files.map(async fullPath => {
    ...
    // Extract query parameters
    const queryParams: { name: string; required: boolean; description?: string }[] = [];
